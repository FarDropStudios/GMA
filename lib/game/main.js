...
MyGame = ig.Game.extend({
	...
	draw: function() {
		...
		if(ig.game.currentLevel === LevelBedroom) {
			this.gmaFont.draw("can you email the cat for me? < >", 350, 600, ig.Font.ALIGN.CENTER);
			this.gmaFont.draw("walk with arrow keys to yr gma.", 350,550, ig.Font.ALIGN.CENTER);
		}
		else if (ig.game.currentLevel === LevelJunkRoom) {
			this.gmaFont.draw("WASD to atk", 350, 600, ig.Font.ALIGN.CENTER);
			this.gmaFont.draw("press r to restart ", 350, 550, ig.Font.ALIGN.CENTER);
		} 
		else if (ig.game.currentLevel === LevelShopRoom) {
			this.gmaFont.draw("its dangerous to go alone", 350, 600, ig.Font.ALIGN.CENTER);
		}
		else if (ig.game.currentLevel === LevelLivingRoom) {
			this.gmaFont.draw("watching my program-shut these dogs up.", 355, 600, ig.Font.ALIGN.CENTER);
		}
		else if (ig.game.currentLevel === LevelKitchen) {
			this.gmaFont.draw("can you fix something for supper? Put it on the oven.", 400, 600, ig.Font.ALIGN.thisCENTER);
		}
		else if (ig.game.currentLevel === LevelBossFight) {
			this.gmaFont.draw("did u feed that to poor gma y did u", 350, 50, ig.Font.ALIGN.CENTER);
			this.gmaFont.draw("did u feed that to poor gma y did u", 350, 0, ig.Font.ALIGN.CENTER);
			this.gmaFont.draw("did u feed that to poor gma y did u", 350, 100, ig.Font.ALIGN.CENTER);
		}
		else if (ig.game.currentLevel === LevelEndRoom) {
			this.gmaFont.draw("can you email my cat to me?", 350, 50, ig.Font.ALIGN.CENTER);
		}
	}
});
...
// up by a factor of 2
ig.main( '#canvas', MyGame, 60, 934, 630, .75);
